...
void solve(){
    ...
    f(1,i,n+1){
        ...
        f(0,j,ki){
            int sij; cin >> sij;
            sets[i].push_back(sij);
            belongings[sij].push_back(i);
            occ[sij] += 1;
        }
    ...
    f(1,i,51){
        int part = c;
        auto help_vec = occ;
        f(0,j,belongings[i].size()){
            int set_index = belongings[i][j];
            f(0,k,sets[set_index].size()){
                int el = sets[set_index][k];
                help_vec[el] -= 1;
                if(help_vec[el] == 0) part -= 1;
            }
        }
        if(part != c) ans = max(ans,part);
    }
