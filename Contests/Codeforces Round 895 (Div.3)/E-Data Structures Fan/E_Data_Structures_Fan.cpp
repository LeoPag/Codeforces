...
void solve(){
    ...
    f(1,i,n+1){
        ll ai; cin >> ai;
        prefix_xor[i] = prefix_xor[i-1] ^ ai;
    }
    ...
    f(1,i,n+1){
        if(s[i-1] == '0') ans[0] ^= (prefix_xor[i] ^ prefix_xor[i-1]);
        else ans[1] ^= (prefix_xor[i] ^ prefix_xor[i-1]);
    }
    ...
    f(0,i,q){
        ...
        if(tp == 2){
            ...
            int l,r; cin >> l >> r;
            ans[0] ^= (prefix_xor[r] ^prefix_xor[l-1]);
            ans[1] ^= (prefix_xor[r] ^prefix_xor[l-1]);
        }
    }
