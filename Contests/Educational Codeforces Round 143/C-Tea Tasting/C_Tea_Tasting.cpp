...
void solve(){
   ...
   f(1,i,n+1) {
    cin >> b[i];
    prefix_sums[i] = b[i] + prefix_sums[i-1];
   }


   VI differentials(n+2,0);
   VL remainder(n+2,0);
   ...
   f(1,i,n+1){
        ...
        if(best != -1){
            differentials[i] += 1;
            differentials[best+1] -= 1; 
            remainder[best+1] += tea_q - (prefix_sums[best] - used);
        }
        else{
            remainder[i] += tea_q - (prefix_sums[i-1] - used);
        }
        used += b[i]; 
   }
   ...
   f(1,i,n+1){
        curr += differentials[i];
        cout << curr * b[i] + remainder[i] << " ";
   }
